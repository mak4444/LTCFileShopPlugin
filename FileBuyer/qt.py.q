...
class Plugin(BasePlugin):
    ...
    def new_question(self):
        ...
        fee = ufee 
        while 1:
            quest_tx = self.window.wallet.make_unsigned_transaction(coins, outputs, self.window.config, ufee)
            print('quest_tx=', quest_tx)
            raw_xt = quest_tx.__str__()
            tlen = len( raw_xt ) / 2
            if   abs(fee - ufee * 1000. / tlen) < 100 :
                break
            ufee = ufee * 1000. / tlen
        ...
        '''
        print('g_raw_xt', raw_xt)
        rquest_tx = Transaction(raw_xt)
        rraw_xt = rquest_tx.__str__()      
        print('g_rraw_xt', rraw_xt)
        
        print('quest_tx.inputs=',quest_tx.inputs())
        print('rquest_tx.inputs=',rquest_tx.inputs())
        '''                
        ...
        #is_relevant, is_mine, v, fee = self.window.wallet.get_wallet_delta(quest_tx)
        #is_relevant, is_mine, v, fee = self.window.wallet.get_wallet_delta(rquest_tx)
